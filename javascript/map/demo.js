	...
	window.demo = { 
		...
		'init': function() {
			...
			this.test('Backbone', function() {
				...
				ForumCollection = Backbone.Collection.extend({ 'url': 'http://ajax.googleapis.com/ajax/services/feed/load?v=1.0&callback=?&q={0}'.format(encodeURIComponent(demo.forum)), 'parse': function(response) { if(response.responseData!=null) return response.responseData.feed.entries; else return null;} });
